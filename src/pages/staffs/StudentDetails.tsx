...
import {
    Edit, Trash, ArrowLeft, Phone, Mail, User, Calendar, MapPin,
    BookOpen, Users, Heart, ChartLine, ClipboardList, Check, X, BookOpenCheck, ChevronLeft, ChevronRight
} from 'lucide-react';
...
const StudentDetails = ({ student, onBack, onEdit, onDelete }: StudentDetailsProps) => {
    ...
    const tabs = [
        ...
        { id: 'termexams', label: 'Term Exams', icon: <BookOpenCheck size={18} /> },
        { id: 'unittests', label: 'Unit Tests', icon: <ClipboardList size={18} /> },
        ...
        switch (activeTab) {
                ...
                return (
                    <div className="space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-sm">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="font-medium text-gray-800">{student.remarks}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-sm">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-1">
                                    ...
                                    <p className="font-medium text-gray-800">{student.religion}</p>
                                </div>
                                <div className="space-y-1">
                                    <label className="text-sm text-gray-400">Caste</label>
                                    <p className="font-medium text-gray-800">{student.caste}</p>
                                </div>
                                <div className="space-y-1">
                                    <label className="text-sm text-gray-400">Category</label>
                                    <p className="font-medium text-gray-800">{student.reservationCategory}</p>
                                </div>
                                <div className="space-y-1">
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-sm">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-1">
                                    <label className="text-sm text-gray-400">House</label>
                                    <p className="font-medium text-gray-800">{student.house}</p>
                                </div>
                                <div className="space-y-1">
                                    <label className="text-sm text-gray-400">Street 1</label>
                                    <p className="font-medium text-gray-800">{student.street1}</p>
                                </div>
                                {student.street2 && (
                                    <div className="space-y-1">
                                        <label className="text-sm text-gray-400">Street 2</label>
                                        <p className="font-medium text-gray-800">{student.street2}</p>
                                    </div>
                                )}
                                <div className="space-y-1">
                                    <label className="text-sm text-gray-400">City</label>
                                    <p className="font-medium text-gray-800">{student.city}</p>
                                </div>
                                <div className="space-y-1">
                                    <label className="text-sm text-gray-400">State & PIN</label>
                                    <p className="font-medium text-gray-800">{student.state} - {student.pincode}</p>
                                </div>
                                <div className="space-y-1">
                                    <label className="text-sm text-gray-400">Country</label>
                                    <p className="font-medium text-gray-800">{student.country}</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            case 'parents':
                return (
                    <div className="bg-white p-6 rounded-xl shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {student.parentsInfo.map((parent: ParentInfo) => (
                                <div key={parent.parentId} className="bg-gray-50 p-4 rounded-lg">
                                    <h4 className="font-medium text-gray-700 mb-3">{parent.relationshipWithStudent}</h4>
                                    <div className="space-y-3">
                                        <p className="text-gray-800 font-medium">{parent.name}</p>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full">
                                        ...
                                        <tbody className="divide-y divide-gray-200">
                                            {examScores
                                                .filter((score: ExamScore) => score.examName === exams.find(e => e.ID === selectedExam)?.Name)
                                                .map((score: ExamScore, index: number) => (
                                                    <tr key={index}>
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {unitTestScores.map((score: UnitTestScore, index: number) => (
                                        <tr key={index}>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                {score.testName}
                                            </td>
            ...
            case 'overall':
                const chartData: ChartData[] = [
                    { name: 'Present', value: attendanceData.total_p_days, color: '#10B981' },
                    { name: 'Absent', value: attendanceData.total_a_days, color: '#EF4444' },
                    { name: 'Half Day', value: attendanceData.total_hd_days, color: '#F59E0B' },
                    {
                        name: 'Holiday',
                        value: attendanceData.total_w_days - (
                ...
                return (
                    <div className="space-y-6">
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        onClick={() => {
                                            if (overallMonth === 1) {
                                                setOverallMonth(12);
                                                setOverallYear(prev => prev - 1);
                                            } else {
                                                setOverallMonth(prev => prev - 1);
                                            }
                                        }}
                                    ...
                                    <span className="text-sm font-medium text-gray-600">
                                        {new Date(overallYear, overallMonth - 1).toLocaleString('default', { month: 'long' })} {overallYear}
                                    </span>
                        ...
                        {/* Exams Overview */}
                        <div className="bg-white p-6 rounded-xl shadow-sm">
                            <h3 className="text-lg font-semibold text-gray-800 mb-6">Exams Overview</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {/* Term Exams Summary */}
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        {exams.map(exam => {
                                            const examScoresFiltered = examScores.filter(
                                                score => score.examName === exam.Name
                                            );
                                            const totalMarks = examScoresFiltered.reduce(
                                                (sum, score) => sum + score.marksObtained, 0
                                            );
                                            const maxMarks = examScoresFiltered.reduce(
                                                (sum, score) => sum + score.maxMarks, 0
                                            );
                                            const percentage = maxMarks > 0 ? (totalMarks / maxMarks) * 100 : 0;
                                            const passedSubjects = examScoresFiltered.filter(
                                            ...
                                            return (
                                                <div key={exam.ID} className="bg-gray-50 p-4 rounded-lg">
                                                    <div className="flex justify-between items-start mb-2">
                                                        ...
                                                        <span className={`px-3 py-1 rounded-full text-sm font-medium ${percentage >= 75
                                                    ...
                                                    <div className="mt-2 space-y-2">
                                                        <div className="flex justify-between text-sm">
                                                            ...
                                                            <span className="font-medium">{totalMarks}/{maxMarks}</span>
                                                        </div>
                                                        <div className="flex justify-between text-sm">
                                                            <span className="text-gray-600">Subjects Passed</span>
                                                            <span className="font-medium">{passedSubjects}/{examScoresFiltered.length}</span>
                                                        </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <div className="bg-white p-3 rounded-lg border border-gray-200">
                                                    ...
                                                    <p className="text-xl font-semibold text-green-600">
                                                        {unitTestScores.filter(test => test.isPassed).length}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div className="grid grid-cols-7 gap-x-0.5 gap-y-1">
                                ...
                                {Array.from(
                                    ...
                                    (_, i) => {
                                        ...
                                        const attendance = attendanceData.day_wise_attendance.find(a => a[day]);
                                        const status = attendance ? attendance[day] : null;

                                        return (
                                            <div
                                                key={i}
                                                className={`w-14 h-14 rounded border ${status ? getAttendanceColor(status) : 'border-gray-200'
                                                    } flex flex-col items-center justify-center mx-auto`}
                                            >
                                                <span className="text-sm font-medium leading-none">{day}</span>
                                                {status && <span className="text-xs leading-none mt-1.5">{status}</span>}
                                            </div>
                                        );
                                    }
                                )}
                    ...
                    <div className="bg-white rounded-xl shadow-sm sticky top-6 z-10">
                        <div className="border-b overflow-x-auto">
                            <ul className="flex min-w-max text-sm font-medium">
                                {tabs.map(tab => (
                                    <li key={tab.id} className="flex-shrink-0" style={{ minWidth: '100px' }}>
                                        <button
                                            onClick={() => setActiveTab(tab.id)}
                                            className={`w-full p-3 border-b-2 ${activeTab === tab.id
