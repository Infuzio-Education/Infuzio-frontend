...
const Breadcrumbs: React.FC = () => {
    ...
    const pathSegments: string[] = location.pathname.split('/').filter(segment => segment && segment !== 'superAdmin');

    const getPath = (index: number): string => '/' + pathSegments.slice(0, index + 1).join('/');
